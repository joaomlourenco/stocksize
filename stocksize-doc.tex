...
\begin{tikzpicture}[remember picture, overlay]
  ...
  % Offsets: 1cm from margins
  \coordinate (H0) at ($(NW)+(0,-1cm)$); % horizontal ruler baseline
  \coordinate (V0) at ($(NW)+(1cm,0)$);  % vertical ruler baseline
...
The \ssname\ package provides \textbf{robust control of the physical PDF page size} (the \textbf{stock size}) inside a LaTeX document.

It complements the \texttt{geometry} package by managing \textbf{paper size transitions} reliably, including \textbf{nested changes}, while delegating margin and layout calculations to \texttt{geometry}.

The package is designed for documents where the \textbf{physical page size itself must change mid-document}, such as appendices, inserts, or mixed-format publications.
...
\begin{description}
  \item[\texttt{patch-geometry}] (boolean, default: \texttt{false}) — When enabled, this option patches the standard \cs{newgeometry}, \cs{restoregeometry}, and \cs{loadgeometry} commands to automatically patch-geometry the physical stock size whenever the layout changes.
\end{description}
...
\begin{description}
  ...
  \item [Nested layouts:] Implements a Last-In-First-Out (LIFO) stack for page geometries, which allows nesting multiple \cs{newstocksize} (or \cs{newgeometry}) calls and restore them in reverse order.
...
\LaTeX{} Project Public License, version 1.3c or later. The latest version is available at \url{https://www.latex-project.org/lppl/}.

\section{Version History}

\begin{description}
   \item[v2.0.0 — December 2026] Major rewrite. Fixed problem with the location of the page numbers after a \cs{restorestocksize} command.
   \begin{itemize}
